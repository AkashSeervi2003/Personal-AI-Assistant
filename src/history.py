...
class UserHistoryManager:
    ...
    def migrate_existing_data(self):
        ...
        try:
            ...
            # Get all existing history
            cur = self.conn.execute("SELECT username, question, answer, timestamp FROM history ORDER BY username, timestamp")
            existing_data = cur.fetchall()

            if not existing_data:
                return  # No data to migrate

            # Group by username and create conversations
            current_user = None
            ...
            for username, question, answer, timestamp in existing_data:
                if username != current_user:
                    ...
                    cur = self.conn.execute(
                        ...
                        (username, f"Conversation {datetime.now().strftime('%Y-%m-%d %H:%M')}", timestamp)
                    )
